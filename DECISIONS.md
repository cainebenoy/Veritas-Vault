...
- **Simplicity First**: The main user interaction is a single input field and a button. We intentionally kept this as simple as possible to maximize usability.
...
- **Search & Filtering**: A debounced search input allows for quick filtering of the archives by title. This uses Firestore's real-time querying capabilities, so search results also update instantly as the user types.
...
- **Content and Metadata**: The detail page shows both the archived content itself and the critical metadata (original URL, IPFS link, blockchain transaction hash).
